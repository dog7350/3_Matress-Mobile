...
public class DirectorScript : MonoBehaviourPunCallbacks
{
    ...
    void Start()
    {
        controls.Touch.PrimaryTouchContact.started += _ => PanStart();
        controls.Touch.PrimaryTouchContact.canceled += _ => PanEnd();
    ...
    public void CameraMove()
    {
        ...
        if (Input.GetKey(KeyCode.P) || (joystick.Buttons.Reset.WasReleasedThisFrame() && SubCamera.enabled)) SubCamera.transform.position = new Vector3(rcx, rcy, rcz);
        ...
        if (ChatInput.isFocused == false)
        {
            ...
            if (Input.GetKey(KeyCode.B) || (joystick.Buttons.Reset.WasReleasedThisFrame() && MainCamera.enabled))
            {
    ...
    IEnumerator PanDetection()
    {
        ...
        previousPosition = controls.Touch.PrimaryFingerPosition.ReadValue<Vector2>();
        while (true)
        {
            position = controls.Touch.PrimaryFingerPosition.ReadValue<Vector2>();
            if (!controls.Touch.SecondaryTouchContact.IsPressed() && buttonPressed() == false)
            {
                if (SubCamera.enabled)
                {
                    SubCamera.transform.Translate(new Vector3((position.x - previousPosition.x) * -0.3f, (position.y - previousPosition.y) * -0.3f, 0));
                }
                else
                {
                    MainCamera.GetComponent<SmoothFollow>().height -= (position.y - previousPosition.y) * 0.01f;
                }
            }
    ...
    IEnumerator ZoomDetection()
    {
        ...
        while(true)
        {
            if (buttonPressed() == false)
            {
                ...
                {
                    if (SubCamera.enabled) SubCamera.transform.Translate(new Vector3(0, 0, 1));
                    else MainCamera.GetComponent<SmoothFollow>().distance -= 0.04f;
                }
    ...
    bool buttonPressed()
    {
        if (joystick.Buttons.Move.IsPressed() || 
            joystick.Movement.TurretLeft.IsPressed() || joystick.Movement.TurretRight.IsPressed() || 
            joystick.Movement.BarrelUp.IsPressed() || joystick.Movement.BarrelDown.IsPressed() ||
            joystick.Buttons.Skip.IsPressed() || joystick.Buttons.Camera.IsPressed() ||
            joystick.Buttons.Power.IsPressed() || joystick.Buttons.Reset.IsPressed() ||
            joystick.Buttons.Item1.IsPressed() || joystick.Buttons.Item2.IsPressed())
        {
            return true;
        }
        return false;
