...
public class PlayerScript : MonoBehaviourPunCallbacks
{
    ...
    {
        instance = this;
        joystick = DirectorScript.instance.joystick;
    }
    ...
    void Update()
    {
        ...
        Vector2 movement = joystick.Movement.TankMove.ReadValue<Vector2>();
        jv = movement.y;
        jh = movement.x;
        ...
        if (Input.GetKeyDown(KeyCode.LeftControl) || joystick.Buttons.Camera.WasReleasedThisFrame()) DirectorScript.instance.CameraChange(camera = !camera);
    ...
    void PlayerController()
    {
        ...
        if (DirectorScript.instance.ChatInput.isFocused == false)
        {
            if (shotCount == 1)
            {
                if (Input.GetKey(KeyCode.Space) || joystick.Buttons.Power.IsPressed())
                {
                    DirectorScript.instance.POWER.fillAmount += (0.3f * Time.deltaTime);
                    PV.RPC("ShotPower", RpcTarget.All);
                }
                if (Input.GetKeyUp(KeyCode.Space) || joystick.Buttons.Power.WasReleasedThisFrame())
                {
            ...
            if (moveCount > 0)
            {
                if (Input.GetKey(KeyCode.UpArrow) || Input.GetKey(KeyCode.DownArrow) ||
                    ...
                    joystick.Movement.TankMove.IsPressed())
                {
            ...
            if (Input.GetKeyUp(KeyCode.UpArrow) || Input.GetKeyUp(KeyCode.DownArrow) ||
                ...
                joystick.Movement.TankMove.WasReleasedThisFrame())
                BGM.Stop();

            if (Input.GetKey(KeyCode.W) || joystick.Movement.BarrelUp.IsPressed()) PV.RPC("BarrelMove", RpcTarget.All, "W");
            if (Input.GetKey(KeyCode.S) || joystick.Movement.BarrelDown.IsPressed()) PV.RPC("BarrelMove", RpcTarget.All, "S");
            if (Input.GetKey(KeyCode.A) || joystick.Movement.TurretLeft.IsPressed()) PV.RPC("TurretMove", RpcTarget.All, "D");
            if (Input.GetKey(KeyCode.D) || joystick.Movement.TurretRight.IsPressed()) PV.RPC("TurretMove", RpcTarget.All, "A");

            if (Input.GetKey(KeyCode.Alpha1) || joystick.Buttons.Item1.WasReleasedThisFrame())
            {
                itemFlag = true;
                if (userNumber == 1)
                    if (ENBVar.p1Item1 != 0)
            ...
            if (Input.GetKey(KeyCode.Alpha2) || joystick.Buttons.Item2.WasReleasedThisFrame())
            {
                itemFlag = true;
                if (userNumber == 1)
                    if (ENBVar.p1Item2 != 0)
            ...
            if (Input.GetKeyDown(KeyCode.Tab) || joystick.Buttons.Skip.WasReleasedThisFrame())
            {
                turn = false;
